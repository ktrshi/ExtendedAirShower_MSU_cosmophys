...
def minim(idx, flag='func'):
    ...
    match flag:
        ...
        case 'func_':
            ...
            m_.limits = [(0, None), (0, None), (0, None), (-100., 100.), (-100., 100.)]
        case 'func1':
            m_ = Minuit(fcn=error1, p0=10000, p1=10, p2=1, p3=0.1, x0=0, y0=0)
            m_.limits = [(0, None), (0, None), (0, None), (0, None), (-100., 100.), (-100., 100.)]
    # print(m_.migrad())
    m_.migrad()
    print(m_.valid)
    fcn_ = m_.fval
...
s = np.zeros(10)
params = []
table_list = list((Path.cwd() / 'EAS' / data_name).glob('**/ch_*'))
for k, table in enumerate(table_list):
    df = pd.read_csv(table, header=None, sep='\s+')
    df_ref = pd.DataFrame(df.values.reshape(-1, 100).tolist())
    data = df_ref.values
...
df_small = pd.read_csv(Path.cwd() / 'EAS' / data_name / 'cnfgs', header=0, sep='\s+',
                       names=['1', '2', '3', '4', '5', '6'],
                       encoding='utf8')
events_data = []
for i in range(100):
...
for i in range(100):
    ...
    m.limits = [(0, None), (0, None), (0, None), (0, None), (-100., 100.), (-100., 100.)]
    # print(m.migrad())
    m.migrad()
    ...
    if i % 20 == 0:
        parametrs = {'m': m, 'rx': rx, 'Q': Q, 'int_func': int_func, 'diff': diff, 'fcn': fcn, 'func': 'func1'}
